...
class StatusViewController: UIViewController {
    ...
    @IBOutlet weak private var flightNumberTextField: UITextField!
    @IBOutlet weak private var callsignControl: UISegmentedControl!
    ...
    var flightNumberHandler: ((_ flightNumber: String?) -> Void)?
    var callsignHandler: ((_ callSign: String?) -> Void)?
    ...
    @IBAction private func callSignSegmentChanged(_ sender: UISegmentedControl) {
        let callSign = sender.selectedSegmentIndex == 0 ? "QFA" : ""
        if let handler = callsignHandler {
            handler(callSign)
        }
    }
    
    @IBAction private func flightNumberEntered(_ sender: UITextField) {
        if let handler = flightNumberHandler {
            handler(flightNumberTextField.text)
        }
